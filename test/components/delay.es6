...
describe("delay", function() {
  it("should send from Emitter to Collector via delay", function *() {
    ...
    yield Network.run(function delay_test() {
      ...
      let delay    = this.proc("sbp/components/delay", null, { "INTVL": 100 * DELAY_SCALE });
  ...
  it("should delay correct amounts", function* () {
    ...
    yield Network.run(function delay_test() {
      ...
      let delay0    = this.proc("sbp/components/delay", "delay0", { "INTVL": 100 * DELAY_SCALE });
      let delay1    = this.proc("sbp/components/delay", "delay1", { "INTVL": 250 * DELAY_SCALE });
      let receiver  = this.proc(Collector((ip) => result.push(ip)));
